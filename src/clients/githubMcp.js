// GitHub MCP outbound 클라이언트. add/commit/push/PR은 이쪽이 담당.
// 봇 토큰 미사용 — 매 요청 사용자 OAuth 토큰으로 일회성 client.
...
async function buildClient(token) {
  const client = new Client({ name: "backend-migration-mcp-as-github-client", version: "0.1.0" });
