...
### §0.5 실제 레포 상태 사전 점검 (Critical — 매 호출 첫 단계)

**모든 scope 호출의 첫 단계로, 코드 생성 전에 target 레포(`woody-rorr/backend`)의 현재 상태를 GitHub MCP 로 직접 읽어 "지금 존재하는 자원 목록" 을 만든다.** 이 단계를 건너뛰면 존재하지 않는 테이블에 FK 를 걸어 부팅 시 `42P01 relation "<x>" does not exist` 로 컨테이너가 무한 재시작한다 (관측 사례: 2026-06-07 comments PR #68 — `posts` FK 참조 → migration 실패 → ECS 크래시 루프 → ALB 가 이전 task 만 살림 → 신규 코드 미반영).

#### 점검 절차 (scope 무관 공통)
1. `mcp__github__get_file_contents` 로 `src/database/migrations/` 디렉토리 목록 조회.
2. 각 `*-create-*-table.ts` 파일명 또는 본문에서 **존재 테이블 화이트리스트**(`existing_tables`) 추출.
...
4. 각 존재 모듈의 `entities/*.entity.ts` 를 읽어 `@Entity('<table>')` 의 `<table>` 명과 컬럼/관계를 수집 → **존재 엔티티·필드 맵**(`existing_entities`) 구성.
...
- **모듈 중복 차단** — `module:<name>` 호출 시 `<name>` 이 이미 `existing_modules` 에 있으면 코드 생성 차단 + `todo: ["spec-fix: <name> 모듈 이미 존재. 수정은 modify:<name> 사용"]`. (재산출은 CREATE TABLE 충돌 유발)
...
#### 호출자 책임
- 호출자(로컬 Claude)는 위 `todo: ["spec-fix: ..."]` 응답을 받으면 **자동으로 다음 호출을 만들지 말고 사용자에게 의존성 누락을 보고**한다. 예: "comments 모듈은 posts 테이블이 필요한데 레포에 없습니다. `module:posts` 먼저 진행할까요?"
